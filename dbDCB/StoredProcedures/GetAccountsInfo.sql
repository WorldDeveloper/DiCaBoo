...
--	(ISNULL((SELECT SUM(Summ) FROM Operations WHERE DebitId.IsDescendantOf(AccountId)=1),0)-ISNULL((SELECT SUM(Summ) FROM Operations WHERE CreditId.IsDescendantOf(AccountId)=1),0)) AS Balance
